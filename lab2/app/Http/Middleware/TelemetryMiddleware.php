...
class TelemetryMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (is_array($decodedResponse) && isset($decodedResponse['error_category'])) {
            $errorCategory = (string) $decodedResponse['error_category'];
        } elseif ($response->getStatusCode() >= 400 && $errorCategory === null) {
